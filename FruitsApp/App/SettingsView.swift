			...
			ScrollView(.vertical, showsIndicators: false) {
				VStack(spacing: 20) {
					...
					GroupBox(
						...
						Divider().padding(.vertical, 4)
						Text("If you wish, you can restart the application by toggle the switch in this box. That way it starts on the onboarding and you will see the welcome screen again.")
							.padding(.vertical, 8)
							.frame(minHeight: 60)
							.layoutPriority(1)
							.font(.footnote)
						...
						Toggle(isOn: $isOnboarding) {
							if isOnboarding {
								Text("Welcome Screen on".uppercased())
									.fontWeight(.bold)
									.foregroundColor(.green)
							} else {
								Text("Show Welcome Screen".uppercased())
									.fontWeight(.bold)
